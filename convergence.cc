...
// TODO(sdk_user): 1. Add the declarations of your method IDs.
  const char* const kConvergence = "Convergence";

// TODO(sdk_user): 2. Implement the methods that correspond to your method IDs.
  std::string Convergence() {
    double an = 0;
    const double answer = M_PI / 4;
    const double eps = 1e-8;
    int k;

    clock_t start = clock();
    for (k = 0; fabs(an - answer) > eps; k++) {
      double delta = 1. / (2*k + 1);
      if (k % 2 == 0) {
        an += delta; 
      } else {
        an -= delta;
    ...
    clock_t end = clock();
    char ret[100];
    sprintf(ret, "an: %.16f  iterated: %d  time: %d (msec)", answer, k, (int)((end - start)/ CLOCKS_PER_SEC));
...
/// invoke the method.
class ConvergenceScriptableObject : public pp::deprecated::ScriptableObject {
 public:
...
bool ConvergenceScriptableObject::HasMethod(const pp::Var& method,
                                             pp::Var* exception) {
  if (!method.is_string()) {
    return false;
  }
  ...
  bool has_method = (method_name == kConvergence);
  return has_method;
}

pp::Var ConvergenceScriptableObject::Call(const pp::Var& method,
                                           const std::vector<pp::Var>& args,
                                           pp::Var* exception) {
  if (!method.is_string()) {
    return pp::Var();
  ...
  // as its method ID.
  if (method_name == kConvergence) {
    return pp::Var(Convergence());
  }
  return pp::Var();
}
...
///     nexes="ARM: convergence_arm.nexe
///            ..."
...
class ConvergenceInstance : public pp::Instance {
 public:
  /// The constructor creates the plugin-side instance.
  /// @param[in] instance the handle to the browser-side plugin instance.
  explicit ConvergenceInstance(PP_Instance instance) : pp::Instance(instance)
  {}
  ...
  /// ConvergenceScriptableObject is the plugin-side representation of that
  /// scriptable object.
  /// @return The browser's handle to the plugin side instance.
  virtual pp::Var GetInstanceObject() {
    ConvergenceScriptableObject* hw_object =
        new ConvergenceScriptableObject();
...
class ConvergenceModule : public pp::Module {
 public:
  ConvergenceModule() : pp::Module() {}
  virtual ~ConvergenceModule() {}

  /// Create and return a ConvergenceInstance object.
  /// @param[in] instance The browser-side instance.
  /// @return the plugin-side instance.
  virtual pp::Instance* CreateInstance(PP_Instance instance) {
    return new ConvergenceInstance(instance);
  }
};
